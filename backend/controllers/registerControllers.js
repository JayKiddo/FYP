...
exports.login = (req,res) => {
	...
	Member.findOne({email: req.body.email}).exec((error,member)=>{
		...
		const token = jwt.sign({_id: member._id}, 'KDA123123KDA', {expiresIn: '1d'})
		//save this token in cookie
		res.cookie('token',token,{expiresIn: '1d'});
		//give json response with user permission and token
		//next time when they make a request to our backend, they can send that token and use that validate the user
